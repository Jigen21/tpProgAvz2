...
import {all} from './OfertaFunctions';
...
export default function DenseTable() {
  ...
  const[ofertasTraidas,setOfertasTraidas] = useState([{}]);

  useEffect(() => {
      const traerItems = async () => {
        const res = await all();
        setOfertasTraidas(res);
      }
      traerItems();
      //console.log(cards);   
  },[]);
    ...
    <div className={classes.root}>
      <Paper className={classes.paper}>
        <Table className={classes.table} size="small">
          ...
          <TableBody>
            {ofertasTraidas.map(row => (
              <TableRow key={row.idOferta}>
                ...
                <TableCell align="right">{row.idProducto}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
